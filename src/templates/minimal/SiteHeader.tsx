  ...
  return (
    <header
      className={[
        ...
        scrolled ? "bg-paper/85 border-b rule-soft" : "bg-paper/65 border-b border-transparent",
      ].join(" ")}
    >
      <div className="mx-auto w-full max-w-[920px] px-5 sm:px-6 md:px-8 flex items-center justify-between h-16">
        <Link
          to="/"
          onClick={() => {
            if (loc.pathname === "/") window.scrollTo({ top: 0, behavior: "smooth" });
          }}
          className="text-[15px] font-semibold tracking-tight hover:text-accent transition-colors"
          aria-label="Md. Mosfikur Rahman, home"
        >
          {profile.shortName} Rahman
        </Link>
              ...
              {({ isActive }) => (
                <span className="relative">
                  ...
                  {isActive && (
                    <span
                      aria-hidden
                      className="absolute -bottom-[5px] left-0 right-0 h-[2px]"
      ...
      {open && (
        <div className="md:hidden border-t rule-soft bg-paper">
          <div className="mx-auto w-full max-w-[920px] px-5 sm:px-6 md:px-8 py-4 flex flex-col gap-3">
            {nav.map((n) => (
              <NavLink
                key={n.to}
                to={n.to}
            ...
            <button
              onClick={toggle}
              className="self-start text-[12px] text-muted-foreground mt-2"
            >
